...
from rope import apply_rotary_emb, RotaryPosEmbed
...
if __name__ == '__main__':
    ...
    import yaml 
    with open('/home/sunyu/galbot/IL_training_codebase/config/deco_libero_80m.yaml', 'r') as f:
        config = yaml.safe_load(f)
    model = modeling(**config['model'])

    imgs = torch.randn(1, 2, 3, 256, 256)
    obs = torch.randn(1, 8)
    act = torch.randn(1, 16, 7)
